...
export default function CollegeCard({
  ...
  return (
    <div
      className="relative group block glass hover:border-cyan/40 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_0_20px_rgba(0,244,254,0.1)] p-5"
    >
      {/* Full-card clickable overlay link */}
      <Link href={`/colleges/${slug}`} className="absolute inset-0 z-0" aria-label={`View details for ${name}`} />

      {/* Content wrapper - clicks pass through to the overlay Link */}
        ...
        <div className="flex items-center justify-between mt-auto pt-2 border-t border-white/5">
          ...
          {onSave && (
            <button
              ...
              aria-label={saved ? `Remove ${name} from saved shortlist` : `Save ${name} to shortlist`}
              className={`pointer-events-auto relative z-10 text-xs px-3 py-1.5 rounded-md transition-all min-h-[36px] flex items-center justify-center font-semibold ${
                saved
